...
{
	NSCharacterSet* numcharacters = [NSCharacterSet characterSetWithCharactersInString:kValidate_Numeric];
...
//    [UIButton appearance].titleLabel.font =UIFONTSYSTEM_REGULAR(IS_IPAD? 22:14);
    ...
    [[UILabel appearance] setSubstituteFontName:UIFONTAPP];
    
    [[UITabBarItem appearance] setTitleTextAttributes:[NSDictionary dictionaryWithObjectsAndKeys:UIFONTSYSTEM_REGULAR(IS_IPAD? 13:13), NSFontAttributeName, nil] forState:UIControlStateNormal];
    
    [[UINavigationBar appearance] setBarStyle:UIBarStyleBlack];
    [[UINavigationBar appearance] setTitleTextAttributes:@{ NSForegroundColorAttributeName:[UIColor whiteColor], NSFontAttributeName:UIFONTSYSTEM_REGULAR(IS_IPAD? 25:17)}];
    
    //Back
    [[UIBarButtonItem appearanceWhenContainedIn:[UINavigationBar class], nil] setTitleTextAttributes:
     @{NSTextEffectAttributeName:[UIColor whiteColor],
       NSFontAttributeName:UIFONTSYSTEM_REGULAR(IS_IPAD? 24:16)} forState:UIControlStateNormal];
...
//    [[UINavigationBar appearance] setBackIndicatorTransitionMaskImage:[UIImage imageNamed:@"Nav_Back"]];
    ...
    CGRect rect = CGRectMake(0.0f, 0.0f, ApplicationDelegate.window.frame.size.width, 64.0f);
    UIGraphicsBeginImageContext(rect.size);
    CGContextRef context = UIGraphicsGetCurrentContext();
    
    CGContextSetFillColorWithColor(context, [MainNavBackColor CGColor]);
...
+(void)applyFontNormalToTextField:(UITextField*)textField withSize:(CGFloat)size
{
    if (IS_IOS_6) {
    ...
    {
        textField.font = kFontHelveticaNeue(size);
        [[textField placeholder] drawInRect:textField.frame withAttributes:@{NSFontAttributeName: kFontHelveticaNeue(12)}];
    }
}
+(void)applyFontNormalToLabel:(UILabel*)label withSize:(CGFloat)size
{
    label.font = kFontHelveticaNeue_Bold(size);
}
+(void)applyFontBoldToLabel:(UILabel*)label withSize:(CGFloat)size
{
    label.font = kFontHelveticaNeue_Bold(size);
}
...
+(BOOL)writeTextToFileAtPath:(NSString *)fileName stringToWrite:(NSString *)stringToWrite
{
    ...
    if (error != nil)
    {
        DLog(@"There was an error in creating a file : %@", [error description]);
        return NO;
    }
    else
    {
...
+(BOOL)createDirectoryAtPath:(NSString *)path
{
    ...
    if (error != nil)
    {
        DLog(@"There was an error in creating a folder : %@", [error description]);
        return NO;
    }
    else
    {
...
+ (void)updateJobCountOnMenu:(NSInteger)count
{
    ...
    [userInfo setValue:[NSNumber numberWithInteger:count] forKey:kNotificationForCurrentCountOfJobs];
    NSNotificationCenter* nc = [NSNotificationCenter defaultCenter];
    [nc postNotificationName:kNotificationForCurrentCountOfJobs object:self userInfo:userInfo];
}
+ (void)updateMessageCountOnMenu:(NSInteger)count
{
    NSMutableDictionary* userInfo = [NSMutableDictionary dictionary];
    [userInfo setValue:[NSNumber numberWithInteger:count] forKey:kNotificationForCurrentCountOfMessages];
    NSNotificationCenter* nc = [NSNotificationCenter defaultCenter];
    [nc postNotificationName:kNotificationForCurrentCountOfMessages object:self userInfo:userInfo];
}
              ...
              animated: (BOOL)animated
{
    ...
    [map setVisibleMapRect:[polygon boundingMapRect] edgePadding:UIEdgeInsetsMake(100.0, 100.0, 100.0, 100.0) animated:animated];
    
}
/**
 *  Get Time for Travel between two CLLocation with avg speed 28.8 kmph
 *
 *  @param source      source CLLocation
 *  @param destination destination CLLocation
 *
...
/**
 *  Get Distance in KM between two CLLocation
 *
 *  @param source      source CLLocation
 *  @param destination destination CLLocation
 *
 *  @return CGFloat kilometers
 */
